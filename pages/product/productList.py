...
def searchProduct() :
    ...
    # 검색조건 > 파트너사 > '전체' 선택
    element.ClickByNameIndex('partnerRadioType', 'ALL', '1-2')
    # 검색조건 > 검색어 > '상품명' + 인풋박스 공란
    element.SelectBoxByName('searchKeywordType', '상품명', '1-3')
    element.InputByName('searchKeywordInput', '', '1-4')
    # 검색조건 > 기간 > '상품등록일' + '1개월' 선택
    element.SelectBoxByName('periodInfo', '상품명', '1-5')
    element.ClickByXPath('//*[@id="searchForm"]/table/tbody/tr[5]/td/div/span[1]/button[3]', '1-6')
    # 검색조건 > 상품상태 > '승인상태' + 체크박스 전체 선택
    element.SelectBoxByName('applyStatus', '승인상태', '1-7')
    # 검색조건 > 카테고리 > '전시 카테고리' + 1 Depth 선택
    element.SelectBoxByXpath('applyStatus', '전시카테고리', '1-8')
    # 검색조건 > [검색] 버튼 클릭
...
def searchProductReset() :
    ...
    # 검색조건 > 파트너사 > '전체' 선택
    element.ClickByNameIndex('partnerRadioType', 'ALL', '2-2')
    # 검색조건 > 검색어 > '상품명' + 인풋박스 공란
    element.SelectBoxByName('searchKeywordType', '상품명', '2-3')
    element.InputByName('searchKeywordInput', '', '2-4')
    # 검색조건 > 기간 > '상품등록일' + '1개월' 선택
    element.SelectBoxByName('periodInfo', '상품명', '2-5')
    element.ClickByXPath('//*[@id="searchForm"]/table/tbody/tr[5]/td/div/span[1]/button[3]', '2-6')
    # 검색조건 > 상품상태 > '승인상태' + 체크박스 전체 선택
    element.SelectBoxByName('applyStatus', '승인상태', '2-7')
    # 검색조건 > 카테고리 > '전시 카테고리' + 1 Depth 선택
    element.SelectBoxByXpath('applyStatus', '전시카테고리', '2-8')
    # 검색조건 > [검색] 버튼 클릭
    element.ClickByXPath('//*[@id="searchForm"]/table/tbody/tr[2]/td[2]/div/button[2]', '2')

# 상품 리스트 > 검색조건 df 검색 후 상품 수정 (TC_3) *** 수정 후 검증 필요
def productModify() :
    # 검색조건 df로 검색 *** 추후 신규 상품 등록한 후 그걸 수정 확인 하는 것으로 변경 필요
    ...
    # 상품명 수정
    element.InputByName('productName', 'PL_test_tc_3', '3-3')
    # 판매가 수정
...
# 상품 리스트 > 검색조건 df 검색 후 상품 복사 등록 (TC_4) *** 수정 후 검증 필요
def productCopy():
