...
def inputBI() :
    ...
    # 기본정보 > 회사명
    companyName = lib.create_driver.driver.find_element(by=By.XPATH,
    ...
    representativeName = lib.create_driver.driver.find_element(by=By.XPATH,
                                                               value= BI + 'tr[1]/td[2]/div/div/input')
    representativeName.clear()
    representativeName.send_keys('테스트')

    # 기본정보 > 사업자등록번호
    companyRegistrationNumber = lib.create_driver.driver.find_element(by=By.XPATH,
                                                                      value= BI + 'tr[2]/td/div/div/input')
    companyRegistrationNumber.clear()
    companyRegistrationNumber.send_keys('0000000000')

    # 기본정보 > 대표전화번호
    representativePhoneNumber = lib.create_driver.driver.find_element(by=By.XPATH,
    ...
    representativeEmail2 = lib.create_driver.driver.find_element(by=By.XPATH,
                                                                 value= BI + 'tr[4]/td[1]/div/div[2]/div/input')
    representativeEmail2.clear()
    representativeEmail2.send_keys('nhnsoft.com')

    # 기본정보 > 팩스번호
    faxNumber = lib.create_driver.driver.find_element(by=By.XPATH,
                                                      value= BI + 'tr[4]/td[2]/div/div/input')
    faxNumber.clear()
    faxNumber.send_keys('01011111111')

    # 기본정보 > 업종
    businessType = lib.create_driver.driver.find_element(by=By.XPATH,
    ...
    try :
        # [저장] 버튼 클릭
        save = lib.create_driver.driver.find_element(by=By.LINK_TEXT,
                                                     value='저장')
        action = ActionChains(lib.create_driver)
